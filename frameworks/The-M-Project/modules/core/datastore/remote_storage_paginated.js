                ...
                */
                if(opType === 'create') {
                    if(config.create.receiveIdentifier) {
                        config.create.receiveIdentifier(results, obj.model);
                    } else {
                ...
                */
                if(obj.onSuccess) {
                    if(obj.onSuccess.target && obj.onSuccess.action) {
                        obj.onSuccess = that.bindToCaller(obj.onSuccess.target, obj.onSuccess.target[obj.onSuccess.action], [results]);
                        if(opType === 'read') {
                            that.createModelsFromResult(results, obj.onSuccess, obj);
                        } else {
                            obj.onSuccess();
                        }
